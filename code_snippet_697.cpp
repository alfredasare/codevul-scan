...
bool extractResFilePathParts(const StringPiece16& path, StringPiece16* outPrefix,
    ...
    *outSuffix = (iter != path.end()) ? StringPiece16(iter, path.end() - iter) : StringPiece16();
    *outEntry = StringPiece16(lastOccurence + 1, (iter != path.end()) ? iter - lastOccurence - 1 : path.end() - lastOccurence - 1);

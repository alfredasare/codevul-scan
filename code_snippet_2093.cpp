PHP_FUNCTION(locale_accept_from_http)
{
    ...
    if (http_accept_len > INTL_MAX_LOCALE_LEN) {
        intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR, "locale_accept_from_http: input parameter too long", 0 TSRMLS_CC);
        RETURN_FALSE;
    }

    if (http_accept_len <= 0 || strlen(http_accept) != http_accept_len) {
        intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR, "locale_accept_from_http: invalid input parameter", 0 TSRMLS_CC);
        RETURN_FALSE;
    }

static int fmt_mp_to_sp(const struct v4l2_format *f_mp, struct v4l2_format *f_sp)
{
    ...
    if (pix_mp->width < 0 || pix_mp->height < 0 || pix_mp->pixelformat < 0 || pix_mp->field < 0 || pix_mp->colorspace < 0)
        return -EINVAL;
    ...
    if (pix_mp->plane_fmt[0].sizeimage >= 0 && pix_mp->plane_fmt[0].bytesperline >= 0) {

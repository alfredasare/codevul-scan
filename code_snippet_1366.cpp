...
{
    if (int_idx > MAX_INTERRUPT_VECTOR) {
        VMW_IRPRN("Invalid interrupt vector %u", int_idx);
        return false;
    }

    PCIDevice *d = PCI_DEVICE(s);

    if (s->msix_used && msix_enabled(d) && int_idx < s->msix_vectors) {
        VMW_IRPRN("Sending MSI-X notification for vector %u", int_idx);
        msix_notify(d, int_idx);
        return false;
    } else if (s->msi_used && msi_enabled(d)) {
